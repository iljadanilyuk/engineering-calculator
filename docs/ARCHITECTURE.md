...
This repository defines the Poznyak Engineering Calculator as a web-first full-stack product: shared contracts, one backend, one CSR admin app (`webapp`), one Astro public site (`website`), and little custom infrastructure. Mobile is deferred unless `task.md` adds it later.
...
Do not hand-copy API shapes into clients. When a contract changes, validate producer and consumers in one pass: backend route/service, website usage where present, and webapp API client/form.
...
There are two browser surfaces, split by whether the pages need SEO. `website` (Astro, SSG by default, SSR/hybrid only when needed) owns public, search-indexable, and link-previewed pages: the offer, calculator, project examples, included-work sections, stages, FAQ, contacts, and public proposal views where needed. `webapp` (React CSR) owns screens that live behind sign-in and need no SEO: services/prices, submitted calculations, lead statuses, notes, settings, and admin workflows.

The webapp follows these client rules:

- TanStack Query owns server state.
- TanStack Form owns form state.
- Zod schemas come from `@poznyak-engineering-calculator/contracts`.
...
`website` is a separate Astro workspace for public SSG/SSR pages. Pages prerender to static HTML by default. Freshness should climb this ladder: SSG plus rebuild/redeploy for durable landing/example/FAQ/contact content; cached on-demand/SSR routes with CDN headers such as `stale-while-revalidate` when freshness matters more than a full redeploy cycle; Astro server islands for non-SEO-critical dynamic or personalized fragments; uncached or personalized SSR only for request-specific pages such as token-protected proposal views that must reflect the current request. On-demand/SSR routes and server islands both require an Astro adapter and a runtime-capable deployment; they do not work from a pure Static Site host or object-storage static website. Server islands on cached pages or rolling deploys require a stable secret `ASTRO_KEY` shared by build and runtime environments; never commit it, expose it as `PUBLIC_*`, or bake it into static output. Shared CDN caching is only for anonymous, public-equivalent HTML; auth-dependent or personalized responses must use `private`/`no-store` or a deliberate `Vary: Cookie`/`Authorization` strategy, and `ASTRO_KEY` is not a cache privacy boundary.

SEO-critical content must be present in the initial HTML: titles, descriptions, canonical URLs, social preview tags, product/category names, indexable descriptions, and public prices when snippets need them. Client islands and server islands may enhance the page, but they must not be the only source of SEO-critical content. `website` does not own the full auth flow and should not duplicate the CSR client from `webapp`; auth in `website` is limited to public-site needs such as a logged-in header state or lightweight actions. If the website starts reading API data or shared DTOs, connect `@poznyak-engineering-calculator/contracts` and validate producer/consumer sides the same way as `webapp`.

Astro remains the default website stack because it is content-first, static-first, low-JS by default, and gives agents a clear SEO surface. Choose Next.js only when this project intentionally wants a Vercel-optimized ISR/cache platform. Treat TanStack Start as an optional future React full-stack path for teams that want one React app with selective SSR, not as the baseline here.

## Testing

Backend unit/integration tests verify contracts and auth behavior at the owning layer. Webapp E2E uses Playwright and starts a real backend + Vite through `webServer`.

Client E2E is a happy-path smoke layer, not the place for large validation matrices. Keep negative payloads, password/JWT/session rules, and error-shape checks in backend tests. Add fast client-level tests for form validation and API state edge cases when those surfaces grow.
...
Local PostgreSQL is provided by Docker Compose, not by a native database install. The development service uses `postgres:18-alpine`, exposes `poznyak_engineering_calculator` on host port `54329`, and stores data in the `postgres_18_data` volume. The test service uses the same image with database `poznyak_engineering_calculator_test`; automated runners set `POSTGRES_TEST_PORT` to a repository-derived port when they need isolation. PostgreSQL 18 is intentional here because the backend schema relies on the native `uuidv7()` database function.
