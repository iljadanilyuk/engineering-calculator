...
test('defaultTestDatabaseUrl builds the documented postgres test URL', () => {
  expect(defaultTestDatabaseUrl('55432')).toBe(
    'postgresql://superuser:superpassword@localhost:55432/poznyak_engineering_calculator_test?schema=public',
  )
})
...
test('postgresPortFromDatabaseUrl returns explicit ports and postgres defaults', () => {
  ...
  expect(
    postgresPortFromDatabaseUrl(
      'postgresql://superuser:superpassword@localhost/poznyak_engineering_calculator_test?schema=public',
    ),
  ).toBe('5432')
})
...
test('assertTestDatabaseUrl accepts test databases and rejects development databases', () => {
  ...
  expect(() =>
    assertTestDatabaseUrl(
      'postgresql://superuser:superpassword@localhost:54329/poznyak_engineering_calculator?schema=public',
    ),
  ).toThrow(/Refusing to run tests against non-test database "poznyak_engineering_calculator"/)
})
