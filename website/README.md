...
The website workspace is a separate Astro project for public, SEO-facing surfaces: the offer, calculator, project examples, included-work sections, stages, FAQ, contacts, and token-protected public proposal pages when needed. It is the SSG-first counterpart to the CSR `webapp` admin cabinet. Keep it independent from authenticated browser-app workflows unless a product need explicitly requires shared API data.
...
A route can opt into server rendering (SSR) with `export const prerender = false`. SSR is a deliberate upgrade because it requires installing a Node adapter and deploying as a runtime service instead of a Static Site. Keep stable public copy static. Render only request-specific routes on demand, such as token-protected proposal views or data that must reflect the current request.

Use this freshness ladder before making an entire page uncached or personalized SSR:

1. SSG plus rebuild/redeploy for durable public copy such as the landing page, examples, FAQ, and contact text.
