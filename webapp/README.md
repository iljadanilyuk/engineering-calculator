...
The CSR browser client provides the authenticated admin cabinet for the engineering calculator. It lives behind authentication and needs no SEO, so it stays client-side rendered; the public calculator and landing content live in the `website` workspace instead.

## Project Surface Status

Active for v1 admin work: login, services/prices, leads/calculations, statuses, notes, proposal links, and settings.
...
- TanStack Form
- TanStack Router
- Zod contracts from `@poznyak-engineering-calculator/contracts`
- shadcn CLI
- Playwright
- ESLint
...
The current shadcn configuration is `radix-maia` with the `hugeicons` icon library and CSS variables, as recorded in `components.json`. This project keeps the full official shadcn component registry as a complete local UI foundation. Do not add community registries, blocks, or custom UI generator output unless the product asks for them.
...
Use the local `shadcn` devDependency pinned in `webapp/package.json` and `bun.lock`; do not use `shadcn@latest` for routine refreshes because it can produce registry output that no longer matches this project. If generated files need compatibility fixes for current package versions, keep the edits small and leave app-specific composition outside `src/components/ui`.
