...
- Before using a new library, inspect the relevant `package.json`. Prefer installed libraries such as Zod, TanStack Query, TanStack Form, Hono, Prisma, and `@poznyak-engineering-calculator/contracts`.
...
- For E2E, use Playwright for web. Read `docs/TESTING.md` before adding flows.
- Mobile is deferred for this product unless `task.md` is updated with a new mobile scope.

## Project Context

- Use `README.md` and `task.md` as the source of truth for project setup, product scope, and open decisions.
