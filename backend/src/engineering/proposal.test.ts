...
describe('commercial proposal generation', () => {
  test('creates a self-contained Cyrillic commercial offer artifact with checksum metadata', async () => {
    ...
    expect(artifact.htmlSnapshot).toContain('family=Inter:wght@400;500;600;700&family=Nunito:wght@700;800')
    expect(artifact.htmlSnapshot).toContain('--font-body: "Inter"')
    expect(artifact.htmlSnapshot).toContain('--font-heading: "Nunito"')
