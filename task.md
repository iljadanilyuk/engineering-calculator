...
- Added the public `/offer/` choice page with a working preliminary КП path and a disabled/future-state detailed questionnaire option.
...
- Explicit empty or stale `services=` selections on `/offer/` no longer silently fall back to default services; preliminary КП stays blocked until at least one valid section is selected.
- Public calculator API config loading now reconciles selected services instead of overwriting user changes made before the fetch completes.
...
- Added `public_offer_preliminary` source visibility for normal calculation leads in the admin list/detail as `Предварительное КП`.
...
- `bun run test:backend:unit` passed.
- `bun run test:backend:integration` passed.
- `bun run --cwd webapp lint` passed.
- `bun run test:webapp` passed.
- `bun run typecheck` passed.
- Browser smoke against the built website with a temporary Bun static server passed:
  - empty service selection preserves `services=` and shows `Разделы не выбраны`;
  - stale explicit `services=old-id` does not default to unrelated services;
  - single `warm-floor` selection does not add extra default services;
  - detailed questionnaire CTA is disabled while the wizard remains future scope;
  - mobile `/offer/` has no horizontal overflow.

Review log:

- Cicero initial review: 7/10, required fixes for top CTA routing and future-scope questionnaire state.
- Dalton initial review: 7/10, required fixes for empty/stale service selection, API config selection reconciliation, source label, and privacy copy.
- Dalton focused re-review: 8.5/10, required fixes for stale explicit services and active questionnaire handler.
- Dalton second focused re-review: 9.3/10, no blockers; residual source visibility note found.
- Dalton final focused re-review: 9.6/10, required fixes none; gate cleared.
